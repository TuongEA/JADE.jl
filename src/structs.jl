...
mutable struct JADESolveOptions
    ...
    fractionMC::Float64                                                         # fraction of forward passes using monte-carlo inflow sequences.
    ...
    write_eohcuts::Bool                                                         # if true a steady-state model will write a set of files to Input/<data_dir>/EOH.
end

# An object containing decision rule can be applied to spill (MWh) and generation (MWh) at a hydro station.
struct DecisionRule
    slope::Real        # the slope, w.r.t. the current storage in `reservoir`, of the (linear) bound applied to generation and / or spill flow corresponding to `station`. Units are MWh / Mm³.
    intercept::Real    # the bound applied to generation and / or spill flow corresponding to `station` when `reservoir` is at 0 storage.  Units are (MWh).
    boundtype::Symbol  # set to `:upper`, `:lower`, or `:equality` depending on how the flow should be restricted.
    station::Symbol    # the hydro station that the decision rule restricts.
    reservoir::Symbol  # the hydro reservoir whose storage level affects the bound applied.
    flowtype::Symbol   # set to `:generation`, `:spill`, or `:combined`.
    weeks::Vector{Int} # defines the weeks in the year that the decision rule is in effect. E.g. [1:10,43:52].
end

function DecisionRule(  # This function constructs a DecisionRule object
    slope::Real, intercept::Real, boundtype::Symbol, station::Symbol, reservoir::Symbol, flowtype::Symbol,
    ...
    return DecisionRule(slope,intercept,boundtype,station,reservoir,flowtype,convert_to_int_array(weeks))
end
...
mutable struct RunData
    ...
    penalty_lb::Float64                                                         # penalty for flows below lower bound. ($/MWh)
    penalty_ub::Float64                                                         # penalty for flows above upper bound. ($/MWh)
    ...
    use_terminal_mwvs::Bool                                                     # specifying whether we use the given terminal MWVs.
...
# An object containing the quantity and value (cost/price) of a penalty tranche
...
mutable struct JADESimulation
    sim_dir::String                                                             # the folder that the simulation output should be written to (`Output/<data_dir>/<policy_dir>/<sim_dir>`).
    sim_type::Symbol                                                            # the type of simulation, either `:monte_carlo` or `:historical`.
    replications::Int                                                           # the number of replications of the simulation.
    sim_years::Union{Nothing,Vector{Int}}                                       # either `nothing` or a vector of years that should be sampled from in the simulation.
